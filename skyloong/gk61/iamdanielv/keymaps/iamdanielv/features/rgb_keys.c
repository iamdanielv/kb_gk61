            ...
            if (record->event.pressed) {
                switch (rgb_matrix_get_flags()) {
                    ...
                    default: {
                        ...
                        indicator_enqueue(LEFT_ALT_KI, 200, 3, RGB_WHITE); // blink left alt
                        indicator_enqueue(RIGHT_ALT_KI, 200, 3, RGB_WHITE); // blink right alt
                    } break;
                }
            }
            return false;
        ...
        case RM_SPDU:
            if (record->event.pressed) {
                if (rgb_matrix_get_speed() >= (255 - RGB_MATRIX_SPD_STEP)) {
                    ...
                    blink_arrows();
                    indicator_enqueue(QUOT_KI, 200, 4, RGB_RED); // ' - UP
        ...
        case RM_SPDD:
            if (record->event.pressed) {
                if (rgb_matrix_get_speed() <= RGB_MATRIX_SPD_STEP) {
                    ...
                    indicator_enqueue(SCLN_KI, 200, 4, RGB_RED); // ; - DOWN
                    blink_space(false);
        ...
        case RM_SATU:
            if (record->event.pressed) {
                if (rgb_matrix_get_sat() >= (255 - RGB_MATRIX_SAT_STEP)) {
                    // this update would put us at max
                    indicator_enqueue(L_KI, 200, 4, INDICATOR_RGB_DARK_RED); // L - DOWN
        ...
        case RM_VALU:
            if (record->event.pressed) {
                if (rgb_matrix_get_val() >= (RGB_MATRIX_MAXIMUM_BRIGHTNESS - RGB_MATRIX_VAL_STEP)) {
                    indicator_enqueue(DOT_KI, 200, 4, RGB_RED); // . - UP
